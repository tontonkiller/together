...
export default async function GroupDetailPage({
  ...
  // Fetch group + members in parallel — both are needed before authorizing access.
  ...
  if (groupResult.error) {
    console.error('[groups/detail] Group fetch failed:', id, groupResult.error.message);
  }
  ...
  // Run remaining independent queries concurrently.
  const isAdmin = currentMember.role === 'admin';
  const [invitationsResult, eventsResult, eventTypesResult, plansResult, googleSyncedResult] = await Promise.all([
    isAdmin
      ? supabase
          .from('invitations')
      ...
      : Promise.resolve({ data: [] as unknown as PendingInvitation[] }),
    supabase
      .from('events')
  ...
  const invitations = (invitationsResult.data ?? []) as unknown as PendingInvitation[];
  const events = eventsResult.data;
  const eventTypes = eventTypesResult.data;
  const rawPlans = plansResult.data;
  const googleSyncedEvents = googleSyncedResult.data;

  if (plansResult.error) {
    console.error('[groups/detail] Plans fetch failed:', plansResult.error.message);
  }
