...
export async function POST(
  ...
  const { data: plan, error: planError } = await supabase
    .from('plans')
    .select(
      'id, quorum, status, created_by, slots:plan_slots!plan_slots_plan_id_fkey(id, plan_id, start_date, end_date, start_time, end_time, position, created_at, votes:plan_votes(id, slot_id, user_id, available, created_at))',
    )
    .eq('id', planId)
    .maybeSingle();
